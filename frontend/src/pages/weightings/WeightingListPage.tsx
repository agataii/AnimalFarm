...
export default function WeightingListPage() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ['weightings'],
    queryFn: weightingsApi.getAll,
  });

  const deleteMutation = useMutation({
    mutationFn: weightingsApi.remove,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['weightings'] }),
  });

  const handleDelete = (id: number) => {
    if (!window.confirm('Are you sure you want to delete this weighting?')) return;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <h1 className="text-2xl font-bold">Weightings</h1>
        <Link
          to="/weightings/new"
          ...
          <tbody>
            {data?.map((item) => (
              <tr key={item.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">{item.animalName}</td>
                <td className="p-3">
